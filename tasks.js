// js/tasks.js (v6)
// Adds "sticker burst" celebration animation on every successful completion
// - Keeps v5: daily/once completion lock, SFX, points pop, voice (task+desc only), RWD padding
//
// Required asset (optional but recommended):
// /kidstest/assets/sfx/task_success.mp3
...
// Used to trigger a one-time animation after re-render
...
function burstStickers(anchorEl, opts = {}) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const dx = (Math.random() - 0.5) * 180;     // horizontal spread
    const dy = - (90 + Math.random() * 170);    // fly up
...
function renderTasksContent() {
  ...
  const kidState =
    (state.kidData && kidId && state.kidData[kidId]) ? state.kidData[kidId] : { points: 0, lastTaskCompletion: {} };
  ...
  const cardsHtml = tasks.map((task) => {
    ...
    const pointsText = `+${Number(task.points ?? 0)}`;

    return `
      <div class="${cardClass}" data-task-id="${escapeHtml(taskId)}">
        ...
        <div class="flex items-center space-x-3">
          <span class="text-secondary font-extrabold text-2xl whitespace-nowrap">${pointsText}</span>
  ...
  viewContent.innerHTML = `
    <style>
      ...
      .sticker-fly {
        ...
        animation: fly var(--dur) cubic-bezier(.2,1.1,.2,1) both;
      ...
      .mega-pop {
        ...
        animation: mega 0.9s cubic-bezier(.2,1.2,.2,1) both;
      }
      .mega-pop-inner{
      ...
      @keyframes mega {
        0% { transform: translateX(-50%) translateY(-12px) scale(.6); opacity: 0; }
    ...
    <div class="pb-28">
      <div class="p-4 bg-white rounded-xl shadow-md mb-6 flex items-center justify-between gap-3">
        <div class="min-w-0">
          ...
          <p class="text-sm text-gray-500 mt-1">完成任務可累積點數，滿 50 點可以去孵化一顆精靈蛋 🥚</p>
        </div>
        <div id="points-badge" class="shrink-0 px-4 py-2 rounded-2xl bg-secondary/10 text-secondary font-black text-xl">
          🪙 ${points}
        </div>
      </div>
  ...
  // Clear animation marker after a render so it only plays once
...
window.completeTask = async (taskId, points, cycle = "daily", btnEl = null) => {
  ...
  const kidState = (state.kidData && state.kidData[kidId]) ? state.kidData[kidId] : { points: 0, lastTaskCompletion: {} };
  const lastCompletion = kidState.lastTaskCompletion || {};
  const todayStr = todayKey();
  ...
  if (cycle === "daily" && lastDate === todayStr) {
    return showToast("這個每日任務今天已完成囉！", "info");
  }
  if (cycle === "once" && !!ts) {
    return showToast("這個一次性任務已完成（永久）！", "info");
  }
  ...
  try {
    ...
    // Feedback: sound + sticker burst + animation marker
    playSfx(SFX_TASK_SUCCESS);
    lastCompletedTaskIdForAnim = taskId;

    // Sticker burst from the button location
    burstStickers(btnEl, { count: 12 });

    // Optional big popup when crossing 50 threshold
    ...
    if (afterUnlock > beforeUnlock) {
      showToast("你已經累積到可以孵化精靈蛋的點數了！快去精靈頁看看 🥚", "success");
    }
  } catch (error) {
