// js/settings.js
// Fixes requirement #3: remove custom HTML button modal calls (which created extra blank buttons / cancel not working).
// Use showModal(title, html, '儲存', onConfirm) everywhere.
...
import {
    ...
    const list = (state.kids || [])
      .map(
        ...
        <div class="flex items-center justify-between p-3 bg-white rounded-xl shadow-md mb-2 border-2 ${
          ...
          <div class="flex-1 min-w-0 mr-4">
            <p class="font-black text-gray-800 truncate">${escapeHtml(kid.nickname || "未命名")} ${
          ...
          <div class="flex space-x-2 items-center">
            ${
              kid.id !== currentKid?.id
                ? `<button onclick="window.switchKid('${kid.id}')" class="px-3 py-1 bg-secondary/20 text-secondary rounded-lg font-bold hover:bg-secondary/30" title="切換">切換</button>`
                : ""
            }
  ...
  window.showEditKidModal = (kidId = null) => {
    ...
    const html = `
      ...
      <input type="text" id="kidNickname" value="${escapeAttr(kid.nickname || "")}" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">
    ...
    showModal(title, html, "儲存", async () => {
      await saveKid(id);
      closeModal();
    });
  };
  
  async function saveKid(kidId = null) {
    const id = normalizeId(kidId);
  ...
  /** Shared save/delete for tasks/rewards */
  window.saveItem = async (type, data, itemId = null) => {
    ...
    try {
      ...
      console.error(e);
      showToast(`儲存${collectionName}失敗: ${e?.message || e}`, "danger");
    }
  };
  ...
  window.deleteItem = async (type, itemId) => {
    ...
    try {
      ...
      console.error(e);
      showToast(`刪除${collectionName}失敗: ${e?.message || e}`, "danger");
    }
  };
  ...
  window.showEditTaskModal = (taskId = null) => {
    ...
    const isEdit = !!id;
    const task = isEdit ? (state.tasks || []).find((t) => t.id === id) || {} : { cycle: "daily", points: 10 };
  
    const title = isEdit ? `編輯任務: ${task.name || ""}` : "新增任務";
    const html = `
      <label class="block mb-2 font-medium">任務名稱 (必填)</label>
      <input type="text" id="taskName" value="${escapeAttr(task.name || "")}" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">
  
      <label class="block mb-2 font-medium">任務說明</label>
      <textarea id="taskDescription" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">${escapeHtml(task.description || "")}</textarea>
  
      <label class="block mb-2 font-medium">獎勵點數</label>
      <input type="number" id="taskPoints" value="${task.points ?? 10}" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">
  ...
  window.showEditRewardModal = (rewardId = null) => {
    ...
    const isEdit = !!id;
    const reward = isEdit ? (state.rewards || []).find((r) => r.id === id) || {} : { cost: 100 };
  
    const title = isEdit ? `編輯獎勵: ${reward.name || ""}` : "新增獎勵商品";
    const html = `
      <label class="block mb-2 font-medium">商品名稱 (必填)</label>
      <input type="text" id="rewardName" value="${escapeAttr(reward.name || "")}" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">
  
      <label class="block mb-2 font-medium">商品說明</label>
      <textarea id="rewardDescription" class="w-full p-3 border border-gray-300 rounded-xl mb-4 focus:border-accent focus:ring focus:ring-accent/50">${escapeHtml(reward.description || "")}</textarea>
  
      <label class="block mb-2 font-medium">兌換點數</label>
      <input type="number" id="rewardCost" value="${reward.cost ?? 100}" class="w-full p-3 border border-gray-300 rounded-xl focus:border-accent focus:ring focus:ring-accent/50">
    `;
